...
const PlaceOrderScreen = () => {
    ...
    //to fix the mutation error, create updatedCart variable
    const updatedCart = {}
    ...
    //Or cart.cartItems.reduce((acc, item) => acc + item.price * item.qty, 0).toFixed(2)

    updatedCart.itemsPrice = addDecimals(
        cart.cartItems.reduce((acc, item) => acc + item.price * item.qty, 0))
    
    // updatedCart.itemsPrice =  cart.cartItems.reduce((acc, item) => acc + item.price * item.qty, 0).toFixed(2)
    ...
    updatedCart.totalPrice = (
     ...
     const placeOrderHandler = () => {
        dispatch(
            createOrder({
                // cart in the store contains orderItems, shippingAddress, paymentMethod
